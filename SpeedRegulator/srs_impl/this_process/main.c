...
/*!
 * Store the period time for task task1
 */
  __po_hi_milliseconds (&(period), 1000);
  
/*!
 * \brief Making Periodic Task task1
 *
 ...
 * (this_process_task1_k) the second is the period defined in the AADL 
 * model. Third is the task priority ( 1), fourth is the stack size ( 0 
 * bytes) and last is the subprogram executed by the task
 */
  __po_hi_create_periodic_task (this_process_task1_k, &(period), 1, 0, 0, task1_job);
  
/*!
 * Store the period time for task task2
 */
  __po_hi_milliseconds (&(period), 1000);
  
/*!
 * \brief Making Periodic Task task2
 *
 * Make a periodic task according to AADL model requirements. The first 
 * parameter is the task identifier defined in deployment.h 
 * (this_process_task2_k) the second is the period defined in the AADL 
 * model. Third is the task priority ( 1), fourth is the stack size ( 0 
 * bytes) and last is the subprogram executed by the task
 */
  __po_hi_create_periodic_task (this_process_task2_k, &(period), 1, 0, 0, task2_job);
